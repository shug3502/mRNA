...
params.connections_to_break = [1, 1]; %ie don't break anything
end
...
flux_in = zeros(num_time_steps,numel(find(Q(:,1)))); %store flux into oocyte from neighbouring nurse cells 
...
for tau=1:num_time_steps
    flux_in(tau,:) = distn(tau,find(Q(:,1)))*params.flow_rate*params.dt; %[2,3,5,9] are connections into oocyte
...
total_flux = sum(flux_in) % total flux into oocyte from surrounding nurse cells
