function [q_estimate] = summary_statistic_calculator_combined_3D(par_params,num_particles,is_parallel)
   if is_parallel
        params.input_time = 1;
    ...
    l_t = length(time_vec);
    jumps = zeros(N,10^3);
    q_estimate_spatial = zeros(L/delx+1,l_t);
    xpos = zeros(N,10^3);
    ...
    for w=1:l_t
        ...
        q_estimate_spatial(:,w) = Num_in_bins/N/delx; %estimate of q at time T
    end
...
weight=10;
q_estimate = [reshape(q_estimate_spatial,[],1); weight*q_estimate_path];
